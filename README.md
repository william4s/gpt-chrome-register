...
- 从 CPA 或 SUB2API 自动获取 OAuth 授权链接
- 自动完成注册、验证码获取、登录、OAuth 同意页点击和回调校验
- 支持 `Hotmail` 账号池，以及 `QQ / 163 / Inbucket` 网页邮箱链路
- 支持 `DuckDuckGo` 与 `Cloudflare` 两种邮箱生成方式
...
如果页面在提交前额外出现“我同意以下所有各项”这类总同意勾选框，脚本会先尝试自动勾选，再继续点击“完成帐户创建”。
...
content/activation-utils.js 通用激活策略与点击方式适配
...
shared/                    邮件匹配与流程复用逻辑
scripts/hotmail_helper.py  Hotmail 本地 helper
sidepanel/                 侧边栏 UI
tests/                     Node 内置测试
```
